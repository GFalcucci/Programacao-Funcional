...
f1 [x] = [x]
f1 (x:y:xs)
     | isDigit x = replicate (digitToInt x) y ++ f1 xs
     | otherwise = x : f1 (y:xs)
...
f11 [x] = [(x,False,1)]
f11 (x:y:xs)
     | isDigit x = (x,False,1):(y,True,digitToInt x):f11 xs
...
f3 (x:xs)
     | isDigit x =
         let (s,b) = f3 xs
         in (chr (ord 'a' + digitToInt x - 1) : s, True)
     | otherwise =
         let (s,b) = f3 xs
         in (x:s,b)
...
strToInt::String->Int
strToInt [] = 0
strToInt xs = aux xs 1
  where
    aux [] _ = 0
    aux (x:xs) p = digitToInt x * p + aux xs (p*10)